...
export default function HomePage() {
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-10 md:px-6 md:py-12">
      <section className="relative overflow-hidden rounded-3xl border border-border bg-surface p-7 shadow-[0_0_40px_rgba(59,130,246,0.15)] backdrop-blur-xl md:p-10">
      ...
      <section className="grid gap-4 md:grid-cols-2">
        {agentCards.map(card => (
          <Link
            className="group rounded-2xl border border-border bg-surface p-6 shadow-[0_0_30px_rgba(59,130,246,0.12)] transition duration-200 hover:-translate-y-1 hover:border-accent/60 hover:shadow-[0_0_40px_rgba(59,130,246,0.22)]"
            href={card.href}
            key={card.id}
          >
            ...
            <p className="mt-5 text-sm font-medium text-accent-light transition group-hover:text-white">打开 demo →</p>
          </Link>
        ))}
      </section>
        ...
        {lessonCards.map(lesson => {
          ...
          return (
            ...
            >
              ...
              <p className="mt-5 text-sm font-medium text-accent-light transition group-hover:text-white">进入子页面 →</p>
            </Link>
          );
        })}
      </section>
